...
This is the one where I grade my own model in public.
...
Under the hood, the model is skill priors built on DataGolf data with a live stacking layer that re-scores the leaderboard after every round. Rewriting that layer because the random effects were adding noise resulted in one number at Shinnecock: Wyndham Clark's win probability moving from 2.6% to 33.4% after one round.
